...
for i in php bower node unzip wget sed; do
  which $i > /dev/null
  if [ "$?" -ne 0 ]; then
    echo "Miss $i"
    exit
...
echo "CREATE DATABASE \`openvpn-admin\`" | mysql -h db -u root --password="$mysql_root_pass"
echo "CREATE USER $mysql_user@localhost IDENTIFIED BY '$mysql_pass'" | mysql -h db -u root --password="$mysql_root_pass"
echo "GRANT ALL PRIVILEGES ON \`openvpn-admin\`.*  TO $mysql_user@localhost" | mysql -h db -u root --password="$mysql_root_pass"
echo "FLUSH PRIVILEGES" | mysql -h db -u root --password="$mysql_root_pass"
